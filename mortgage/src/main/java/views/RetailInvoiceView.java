	...
	private VerticalLayout getBillingLayout(Table table, CustomItemContainerInterface container, ItemContainerType itemContainerType) {
		...
		itemHolderLayout.addComponent(addNewItemimage);
		 final TreeTable treeTable = new TreeTable("Order Details");
	        treeTable.setContainerDataSource(goldItemContainer);
	        treeTable.setVisibleColumns(new Object[]{"Delete"});
	        treeTable.setColumnCollapsingAllowed(true);
	        treeTable.setColumnExpandRatio("Delete", 1);
	    //itemHolderLayout.addComponent(treeTable);    
